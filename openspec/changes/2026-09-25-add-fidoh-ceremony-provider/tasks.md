...
- [x] 1.1 `crates/vaultwarden/Cargo.toml`: add optional `fidoh` dependency
...
- [x] 1.2 Same crate: `cargo build -p cryptile-vaultwarden --features
      webauthn,fidoh` succeeds (both features on, fidoh authoritative —
      precedence proven by the 2.2 test, not just a compile).
- [x] 1.3 `deny.toml`: add the fidoh git source to the allow-list with the
...
- [x] 2.1 `webauthn.rs` gating widened to
...
- [x] 2.2 Provider routing: `answer_two_factor`'s provider-7 arm dispatches
...
- [x] 2.3 Budget + error mapping: e2e/unit tests pin the mapping table
...
- 3.4: pending user-side hardware smoke — the wiremock seam suite proves
  wire parity, the typed error mapping, and the budget plumbing; it cannot
  prove device I/O. Record the run in docs/webauthn-notes.md
  ("Recorded runs") once a real key is in hand.
